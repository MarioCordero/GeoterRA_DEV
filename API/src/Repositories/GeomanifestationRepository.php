...
final class GeomanifestationRepository extends Repository
{
  ...
  /**
   * Returns all visible manifestations (visibility = 1) with pagination.
   *
   * @param int $page
   * @param int $limit
   * @return array{data: array[], total: int}
  ...
  public function getAllVisible(int $page = 1, int $limit = 20): array
  {
    ...
    $countSql = "SELECT COUNT(*) FROM geomanifestations WHERE visibility = 1";
    $countStmt = $this->execute($countSql);
    $total = (int) $countStmt->fetchColumn();
  ...
  public function getByProvince(int $provinceSnitCode, int $page = 1, int $limit = 20): array
  {
    ...
    $countStmt = $this->execute($countSql, [':province_snit' => $provinceSnitCode]);
    $total = (int) $countStmt->fetchColumn();
    ...
    $sql = "SELECT geomanifestation_id, province_snit_code, canton_snit_code, district_snit_code,
                       ...
                       visibility, created_at, created_by
                FROM geomanifestations
                WHERE province_snit_code = :province_snit AND visibility = 1
                ORDER BY created_at DESC
                LIMIT :limit OFFSET :offset";
  ...
  public function create(array $data, string $userId): string
  {
    ...
    $finalName = trim($data['name']) !== '' ? $data['name'] : 'SOLI-' . strtoupper(substr($id, -5));
  ...
  public function update(string $id, array $fields): bool
  {
    ...
    $sql = "UPDATE geomanifestations SET " . implode(', ', $sets) . " WHERE geomanifestation_id = :id";
   ...
   * @return bool
   */
  public function updateVisibility(string $id, bool $visible): bool
  {
    return $this->update($id, ['visibility' => $visible ? 1 : 0]);
  }

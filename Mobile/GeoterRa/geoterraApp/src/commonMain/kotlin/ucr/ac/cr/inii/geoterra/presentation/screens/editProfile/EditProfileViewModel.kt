...
class EditProfileViewModel(
  ...
  private fun validateFields(): Boolean {
    ...
    else if (!payload.email.matches(emailRegex)) errors["email"] = "El formato del correo electrónico no es válido."

    if (payload.phone_number?.isNotBlank() == true) {
      val phoneRegex = "^[0-9]{8}$|^[0-9]{4}[- ]?[0-9]{4}$".toRegex()
      if (!payload.phone_number.trim().matches(phoneRegex)) errors["phone"] = "Debe ser un número de teléfono válido."
    }
